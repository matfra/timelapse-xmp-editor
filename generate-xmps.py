...
parser = argparse.ArgumentParser(description='Generate xmp files for developing many photos with similar or gradual settings from 2 DNG files')
parser.add_argument( 'directory', type=str,
        help='working directory containing RAW pictures and the 2 edited extremities DNG files')
parser.add_argument('pics_extension', type=str,
        help='Extension of the RAW pictures. Default : ARW')
...
def build_diff_keys(first_xmp, last_xmp):
    ...
    for first_xmp_key, first_xmp_value in first_xmp_values.items():
        if first_xmp_key in last_xmp_values or isinstance(first_xmp_value, float) is False:
    ...
    for line in base_xmp:
        ...
        if 'crs:' in line:
            ...
            if key in modified_keys and value is not None:
                ...
                interpolated_value = values[0] + ((pic_index + 1 ) * ( values[1] - values[0] ) / pics_count)
...
def main(args):
    ...
    if len(xmps_paths) == 0:
        raise FileNotFoundError("Could not find any xmp files in {}".format(args.directory))
    first_xmp = read_file_to_list_of_lines(xmps_paths[0])
    last_xmp = read_file_to_list_of_lines(xmps_paths[-1])

    pics_paths = get_filespath_in_dir_with_extension(args.directory, args.pics_extension)
    ...
    for pic_path in pics_paths:
        ...
        if new_xmp_filename == xmps_paths[0]:
            ...
            continue
        new_xmp_content=build_final_xmp(first_xmp, modified_keys, pics_count, pic_index)
